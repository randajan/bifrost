...
| `createBeam` | Creates interface for easy state sharing | `channel` - Name of the used channel.<br>`stateAdapter` - *description bellow* | `Beam` - Instance of the interface. |
...
| `createBeam` | Creates interface for easy state sharing across sockets groups | `channel` - Name of the used channel.<br>`stateAdapter` - *description bellow*  | `Beam` - Instance of the interface. |
...
All additional arguments will be passed 1:1 to functions provided by stateAdapter object.


### How to define stateAdapter

stateAdapter can be any object that contains __get__ and __set__ property. Both properties are expect to be a function and can be async. 
 - get function should return current state
 - set function should set provided state and return current state
